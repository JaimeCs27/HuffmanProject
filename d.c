...
int main(int argc, char *argv[])
{
   ...
   fread(&cantBook, sizeof(unsigned int), 1, fe);
   printf("%i\n", cant);
   printf("%s\n", filePath);
   printf("%i\n", cantBook);
    ...
    while(1){
      ...
      //   bits |= a;
        ...
        do {
            ...
            if(8 == j)            /* Cada 8 bits */
            {
                fread(&a, sizeof(char), 1, fe); /* Leemos un byte desde el fichero */
            ...
            if(!q->uno && !q->cero)          /* Si el nodo es una letra */
            {
                ...
                cantBook--;                   /* Actualizamos longitud que queda */
                Longitud--;

                q=Arbol;                      /* Volvemos a la raiz del árbol */
            }
            //printf("%i\n",cantBook);
        } while(cantBook);                  /* Hasta que acabe el fichero */
         // Ignorar los bits sobr
